...
int(2*x, x)
int((a*x+b)*exp(a*x), x)
int((a*x+b)*sin(c*x+d), x)
D(y, x)
dsolve(D(y,x)+y=x, y, x)
subs(x^2+y, x, 3)
```

`D(y,x)` 是形式导数节点，用来说明 `y` 是 `x` 的未知函数；它与立即计算导数的
`diff(y,x)` 不同。`dsolve(equation,y,x)` 当前求解一阶线性方程
`a(x)D(y,x)+b(x)y=c(x)`，返回规则列表，例如：

```text
dsolve(D(y,x)+y=x, y, x)
-> {y -> (_C1 + exp(x)*(x - 1))/exp(x)}
```

积分常数会自动选择未被输入占用的 `_C1`、`_C2` 等名称。如果积分因子中的积分
暂时没有初等闭式，结果会保留 `integrate(...)` 节点。非线性、可分离、高阶和微分
方程组目前尚未由 `dsolve` 处理。

`int`/`integrate` 使用一条 Risch 风格的递归管线：有理函数先做多项式除法和
Hermite/部分分式归约；指数扩张求解 `R' + g'R = f`，并对有理 `f` 的重复极点
执行超指数 Hermite 归约；线性参数的三角和双曲扩张求解对应的成对多项式微分
方程。系数域允许包含与积分变量无关的符号常量，
例如上例中的 `a`、`b`、`c` 和 `d`。对数导数、换元链和 `Si`、`Ci`、`Ei`、
`erf`、`erfi` 等特殊函数也会在同一管线中处理。

平方自由的三次、四次不可约有理分母使用代数根留数公式生成精确对数项；实根
使用 `ln(abs(x-r))`，复根使用复对数。

这还不是完整的一般 Risch 算法：次数超过四次的不可约分母仍缺少
