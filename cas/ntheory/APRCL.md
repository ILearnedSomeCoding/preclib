...
关系归约；系数乘积先累加，最后取模。平方利用对称性减少大整数乘法。
...
最后候选因子次数。`cas_aprcl_options` 提供参数表上界、圆分阶上界、试除界和
辅助素数搜索预算。默认 `t<=720720`、`p^k<=64`，选表时只使用允许阶的字符。
...
`--large` 额外证明 `10^100+267`。测试文件直接包含实现以检查内部运算，
